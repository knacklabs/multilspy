...
class Phpactor(LanguageServer):
    ...
    def setup_runtime_dependencies(self, logger: "MultilspyLogger") -> str:
        ...
        php_ls_dir = os.path.join(os.path.dirname(__file__), "static", "phpactor")
        phpactor_executable_path = os.path.join(php_ls_dir, dependency["binaryName"])
        ...
        if not os.path.exists(php_ls_dir):
            ...
            FileUtils.download_file(
                logger,
                dependency["url"],
                phpactor_executable_path,
            )

        assert os.path.exists(phpactor_executable_path)
        os.chmod(
            phpactor_executable_path,
            os.stat(phpactor_executable_path).st_mode
            | stat.S_IXUSR
            | stat.S_IXGRP
            | stat.S_IXOTH,
        )
    ...
    async def start_server(self) -> AsyncIterator["Phpactor"]:
        ...
        async with super().start_server():
            ...
            self.logger.log(
                f"Received initialize response from phpactor: {init_response}",
                logging.INFO,
            )

            self.logger.log(
                "Sending initialized notification to phpactor",
                logging.INFO,
            )
